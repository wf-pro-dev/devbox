<script lang="ts">
  ...
  import type { File, DirEntry } from "../types";
  ...
  $: isExpanded = node.is_dir ? expanded.has(prefix) : false;

  $: hasChildren = node.file_count && node.file_count > 0;
  $: isExpandable = hasChildren || node.is_dir;
  ...
  $: isOpen = node.is_dir ? isExpanded : virtualOpen;
  ...
  const handleFileDelete = (file: File) => {
    if (dirEntries[prefix]) {
      dirEntries[prefix] = dirEntries[prefix]!.filter(
        (e) => e.file?.id !== file.id,
      );
    }
    onFileDelete(file);
    ...
    {#if node.is_dir}
      <div class="node-actions" on:click|stopPropagation>
        ...
        <button
          ...
          on:click={(e) => handleDirDelete(prefix, e)}
        >
          <svg viewBox="0 0 16 16" fill="none" width="11" height="11">
            <path
