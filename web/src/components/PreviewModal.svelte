<script lang="ts">
  ...
  function copyContent() {
    navigator.clipboard.writeText(editing ? editorContent : content);
    copied = true;
  ...
  function cancelEditDescription() { editingDescription = false; }

  async function saveDescription() {
    savingDescription = true;
    try {
      const updated = await api.editMeta(file.id, { description: descriptionDraft });
  ...
  function onKey(e: KeyboardEvent) {
    if (e.key === 'Escape' && !editing && !showDeliver && !showUpdateModal) dispatch('close');
  }
  ...
  <div class="modal" on:click|stopPropagation role="dialog" aria-modal="true">
    ...
    <div class="header">
      <div class="header-left">
        ...
        <span class="lang" style="--c:{langColor(file.language)}">{file.language || 'text'}</span>
        {#if file.version > 1}
      ...
      <div class="header-right">
        ...
        <button class="icon-btn" title="Deliver" on:click={() => showDeliver = true}>
          <svg viewBox="0 0 16 16" fill="none" width="13" height="13">
            <path d="M2 8h10M8 4l4 4-4 4" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/>
          </svg>
        </button>
        <button class="icon-btn danger" title="Delete" disabled={deleting} on:click={deleteFile}>
          <svg viewBox="0 0 16 16" fill="none" width="13" height="13">
    ...
    <div class="tab-bar">
      {#each ['preview', 'meta', 'versions'] as t}
        <button class="tab" class:active={tab === t} on:click={() => onTabChange(t)}>
        ...
        <div class="preview-wrap">
          <div class="preview-toolbar">
            ...
            <div class="toolbar-actions">
              {#if !editing}
                <button class="tool-btn" on:click={copyContent}>
                  {#if copied}
                    <svg viewBox="0 0 12 12" fill="none" width="11" height="11">
                      <path d="M2 6l2.5 2.5L10 3" stroke="#16a34a" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                    Copied
                  {:else}
                    <svg viewBox="0 0 14 14" fill="none" width="11" height="11">
                      <rect x="4" y="4" width="7" height="8" rx="1.2" stroke="currentColor" stroke-width="1.2"/>
                      <path d="M2.5 9.5V2.5a1 1 0 011-1h5.5" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/>
                    </svg>
                    Copy
                  {/if}
                </button>
                ...
                <button class="tool-btn" on:click={() => showUpdateModal = true}>
                  <svg viewBox="0 0 14 14" fill="none" width="11" height="11">
              ...
              {:else}
                ...
                <button class="tool-btn save" on:click={saveEdit} disabled={saving}>
                  {saving ? 'Saving…' : 'Save'}
                </button>
          ...
          <div class="code-body">
            ...
            {:else if editing}
              ...
              ></textarea>
            {:else}
              <pre><code>{content.slice(0, 8000)}{content.length > 8000 ? '\n\n… (truncated)' : ''}</code></pre>
            {/if}
          </div>
        </div>
      ...
      {:else if tab === 'meta'}
        <div class="meta-pane">
          <div class="meta-grid">
            ...
            <span class="mv"><span class="lang" style="--c:{langColor(file.language)}">{file.language || '—'}</span></span>
              ...
              {#if editingDescription}
                <div class="desc-editor">
                  ...
                  <div class="desc-actions">
                    <button class="desc-save" on:click={saveDescription} disabled={savingDescription}>
                      {savingDescription ? '…' : 'Save'}
                    </button>
                    <button class="desc-cancel" on:click={cancelEditDescription}>Cancel</button>
                  </div>
                </div>
              {:else}
                <button class="desc-display" on:click={startEditDescription}>
                  ...
                  {/if}
                  <svg class="desc-edit-icon" viewBox="0 0 14 14" fill="none" width="11" height="11">
                    <path d="M9.5 2.5l2 2L4 12H2v-2L9.5 2.5z" stroke="currentColor" stroke-width="1.2" stroke-linejoin="round"/>
                  </svg>
                </button>
              {/if}
...
<style>
  ...
  .modal {
    ...
    width: min(860px, 94vw); height: 86vh;
  ...
  .lang {
    flex-shrink: 0; font-family: var(--mono); font-size: 10.5px; font-weight: 500;
  ...
  .version-badge {
    ...
    padding: 1px 6px; border-radius: 3px; color: var(--text-3); flex-shrink: 0;
  }
  ...
  .dl-link {
    display: flex; align-items: center; justify-content: center;
    color: inherit; text-decoration: none; width: 100%; height: 100%;
  }
  .close-btn {
    display: flex; align-items: center; justify-content: center;
    width: 30px; height: 30px; background: none; border: none;
  ...
  .close-btn:hover { background: var(--bg-2); color: var(--text); }

  /* Tabs */
  .tab-bar {
    display: flex; align-items: center; border-bottom: 1px solid var(--border);
    padding: 0 18px; background: var(--bg); flex-shrink: 0;
  }
  .tab {
  ...
  .tab-count {
    font-size: 10px; font-family: var(--mono);
    background: var(--bg-3); border: 1px solid var(--border);
    padding: 0px 5px; border-radius: 8px; color: var(--text-3);
  }
  .tab-spacer { flex: 1; }
  .path-crumb {
  ...
  .toolbar-info { font-family: var(--mono); font-size: 11px; color: var(--text-3); }
  ...
  .editing-badge {
    ...
    color: #92400e; padding: 1px 7px; border-radius: 3px; margin-right: 4px;
  }
  .tool-btn {
    display: flex; align-items: center; gap: 4px;
    background: none; border: 1px solid var(--border); border-radius: var(--radius);
    padding: 3px 10px; font-size: 11.5px; color: var(--text-2);
    cursor: pointer; transition: all 0.1s;
  }
  .tool-btn:hover { background: var(--bg-2); border-color: var(--border-2); }
  .tool-btn.save { background: var(--text); color: white; border-color: var(--text); }
  .tool-btn.save:hover:not(:disabled) { background: #3d3c38; }
  .tool-btn:disabled { opacity: 0.4; pointer-events: none; }

  .code-body { flex: 1; overflow: auto; padding: 16px 20px; background: var(--bg); }
  pre {
    margin: 0; font-family: var(--mono); font-size: 12px;
    line-height: 1.65; white-space: pre; color: var(--text);
  }
  code { font-family: inherit; }
  .editor-ta {
    width: 100%; height: 100%; min-height: 300px; border: none; outline: none;
    resize: none; font-family: var(--mono); font-size: 12px; line-height: 1.65;
    color: var(--text); background: white; tab-size: 2;
    padding: 0; /* code-body already has padding */
  }
  ...
  .ml {
    ...
    color: var(--text-3); padding-top: 1px; white-space: nowrap;
  }
  .mv { font-size: 13px; color: var(--text); }
  .mono { font-family: var(--mono); font-size: 12.5px; }
  .sha { font-size: 11px; word-break: break-all; color: var(--text-3); }
  ...
  .desc-display {
    ...
    transition: border-color 0.1s, background 0.1s; color: var(--text);
  }
  .desc-display:hover { border-color: var(--border); background: var(--bg-2); }
  .desc-display:hover .desc-edit-icon { opacity: 1; }
  .desc-text { font-size: 13px; flex: 1; }
  .desc-empty { font-size: 13px; color: var(--text-3); font-style: italic; flex: 1; }
  .desc-edit-icon { flex-shrink: 0; color: var(--text-3); opacity: 0; margin-top: 2px; transition: opacity 0.1s; }
  .desc-editor { display: flex; flex-direction: column; gap: 6px; }
  .desc-input {
    width: 100%; height: 32px; padding: 0 9px;
    border: 1px solid var(--border-2); border-radius: var(--radius);
    font-size: 13px; background: white; outline: none; transition: border-color 0.1s;
  }
  .desc-input:focus { border-color: var(--text); }
  .desc-actions { display: flex; gap: 5px; }
  .desc-save {
    height: 26px; padding: 0 12px; background: var(--text); color: white;
    border: none; border-radius: var(--radius); font-size: 11.5px; font-weight: 500;
    cursor: pointer; transition: background 0.1s;
  }
  .desc-save:hover:not(:disabled) { background: #3d3c38; }
  .desc-save:disabled { opacity: 0.5; pointer-events: none; }
  .desc-cancel {
    height: 26px; padding: 0 10px; background: none;
    border: 1px solid var(--border); border-radius: var(--radius);
    font-size: 11.5px; color: var(--text-2); cursor: pointer; transition: background 0.1s;
  }
  .desc-cancel:hover { background: var(--bg-2); }
  ...
  .tag-pill {
    ...
    border-radius: 4px; font-size: 11px; font-family: var(--mono); color: #2563eb;
  }
  .trm {
  ...
  .tag-add input {
    border: none; outline: none; padding: 3px 8px; font-size: 11px;
    font-family: var(--mono); width: 90px; background: var(--bg-2);
  }
  .tag-add button {
    border: none; background: var(--bg-3); border-left: 1px solid var(--border);
    padding: 2px 8px; font-size: 14px; color: var(--text-2); cursor: pointer; line-height: 1.3;
  }
  .tag-add button:hover:not(:disabled) { background: var(--text); color: white; }
  .tag-add button:disabled { opacity: 0.4; }

  /* Versions */
  .versions-pane { flex: 1; overflow-y: auto; }
  .state-msg { padding: 40px; text-align: center; font-size: 13px; color: var(--text-3); }
  .state-msg.muted { font-style: italic; }
  .versions-table { width: 100%; border-collapse: collapse; font-size: 12.5px; }
  .versions-table th {
