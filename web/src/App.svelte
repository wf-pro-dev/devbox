<script lang="ts">
  ...
  // Column sort
  type SortField = 'file_name' | 'size' | 'created_at' | 'language' | 'uploaded_by';
...
<div class="app">
  ...
  <header>
    ...
    <div class="search-wrap">
      ...
      />
      {#if searchQuery}
        <button class="sc" on:click={() => { searchQuery = ''; loadFiles(); }}>
  ...
  <div class="body">
    <Sidebar
      ...
      on:selectFile={(e) => { previewFile = e.detail; activeMainTab = 'files'; }}
    />
    ...
    <main>
      ...
      <div class="tab-bar">
        <button
          ...
          on:click={() => (activeMainTab = 'files')}
        >Files</button>
        <button
          class="tab-btn"
          class:active={activeMainTab === 'directories'}
          on:click={() => (activeMainTab = 'directories')}
        >Directories</button>
      </div>

      {#if activeMainTab === 'directories'}
        <DirectoriesTab onFileSelect={(f) => { previewFile = f; activeMainTab = 'files'; }} />
        ...
        {#if activeTag || searchQuery}
          <div class="filter-bar">
            {#if searchQuery}
              <span class="chip">
                "{searchQuery}"
                <button on:click={() => { searchQuery = ''; loadFiles(); }}>×</button>
              </span>
            {/if}
            {#if activeTag}
              <span class="chip blue">
                #{activeTag}
                <button on:click={() => { activeTag = ''; loadFiles(); }}>×</button>
              </span>
            {/if}
          ...
          <div class="empty">
            <div class="empty-icon">
              <svg viewBox="0 0 24 24" fill="none" width="40" height="40">
                <path d="M4 4h8l2 2h6v14H4V4z" stroke="currentColor" stroke-width="1.3" stroke-linejoin="round"/>
                <path d="M9 13h6M12 10v6" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/>
              </svg>
            </div>
              ...
              <thead>
                <tr>
                  <th class="th-sort" on:click={() => setSort('file_name')}>
                    Name
                    {#if sortField === 'file_name'}<span class="sort-arrow">{sortDir === 1 ? '↑' : '↓'}</span>{/if}
                  </th>
                  <th>Description</th>
                  <th class="th-sort" on:click={() => setSort('language')}>
                    Language
                    {#if sortField === 'language'}<span class="sort-arrow">{sortDir === 1 ? '↑' : '↓'}</span>{/if}
                  </th>
                  <th>Tags</th>
                  <th class="th-sort th-right" on:click={() => setSort('size')}>
                    Size
                    {#if sortField === 'size'}<span class="sort-arrow">{sortDir === 1 ? '↑' : '↓'}</span>{/if}
                  </th>
                  <th class="th-sort" on:click={() => setSort('uploaded_by')}>
                    By
                    {#if sortField === 'uploaded_by'}<span class="sort-arrow">{sortDir === 1 ? '↑' : '↓'}</span>{/if}
                  </th>
                  <th class="th-sort" on:click={() => setSort('created_at')}>
                    Date
                    {#if sortField === 'created_at'}<span class="sort-arrow">{sortDir === 1 ? '↑' : '↓'}</span>{/if}
                  </th>
                  <th class="th-actions">
                ...
                {:else}
                  {#each sortedFiles as file (file.id)}
                    <FileRow
                      ...
                      on:preview={(e) => { previewFile = e.detail; }}
                    />
                  {/each}
                {/if}
              </tbody>
...
<style>
  ...
  .logo-mark {
    width: 26px; height: 26px; background: var(--text); color: var(--bg);
    font-family: var(--mono); font-size: 11px; font-weight: 500;
    display: flex; align-items: center; justify-content: center;
    border-radius: 5px; letter-spacing: -0.5px;
  }
  .logo-name { font-family: var(--serif); font-size: 17px; letter-spacing: -0.3px; }
  ...
  .th-actions { text-align: right; padding-right: 14px; }

  .sort-arrow { font-size: 10px; margin-left: 3px; }

  .count-badge {
    font-size: 10px; font-family: var(--mono); font-weight: normal;
  ...
  .skel-desc { width: 200px; }
  .skel-lang { width: 60px; }
  ...
  .err {
    ...
    border-radius: var(--radius); color: #dc2626; font-size: 13px;
  }
  ...
  .empty-icon { color: var(--border-2); margin-bottom: 4px; }
  .empty-title { font-family: var(--serif); font-size: 20px; color: var(--text); }
  .empty-sub { font-size: 13px; color: var(--text-3); margin-bottom: 4px; }
</style>